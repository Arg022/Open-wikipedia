...
app.post("/save-article", async (req, res) => {
    ...
    try {
        await axios.post("http://localhost:8000/wikipedia/save-article", { title });
...
app.get("/articles", async (req, res) => {
    try {
        const response = await axios.get("http://localhost:8000/wikipedia/articles");
...
app.get("/edit/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const response = await axios.get(`http://localhost:8000/wikipedia/articles/${id}`);
...
app.post("/edit/:id", async (req, res) => {
    ...
    try {
        await axios.put(`http://localhost:8000/wikipedia/articles/${id}`, { content });
        res.redirect("/articles");
    } catch (error) {
